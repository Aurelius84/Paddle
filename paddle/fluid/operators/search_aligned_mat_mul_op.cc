...
void assign_dims(const DDim& x_dims, const LoD& x_lod, const DDim& y_dims,
                 const LoD& y_lod, CBLAS_TRANSPOSE trans_x,
                 CBLAS_TRANSPOSE trans_y, std::vector<int64_t>& _dims) {
  std::vector<CBLAS_TRANSPOSE> _trans{trans_x, trans_y};
  _dims.resize(3);
...
class CPUSearchAlignedMatMulOPKernel : public framework::OpKernel<T> {
 public:
  ...
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    top->set_lod(top_lod);
    top->Resize(
        framework::make_ddim({static_cast<int64_t>(offset[batch]), _dims[2]}));
    prepare_ff(ctx, _dims);
    call_gemm_batched(ctx, _trans[0], _trans[1], static_cast<int>(_dims[0]),
                      static_cast<int>(_dims[2]), static_cast<int>(_dims[1]),
...
class CPUSearchAlignedMatMulOPGradKernel : public framework::OpKernel<T> {
 public:
  void prepare_bp(const framework::ExecutionContext& ctx,
    ...
    const int batch = bottom0->lod()[0].size() - 1;
    PADDLE_ENFORCE_EQ(_a_addr->dims()[0], batch,
                      "blob should be initialized before bp");
    ...
    for (int i = 0; i < batch; ++i) {
      ...
      c_addr_diff[i] = const_cast<T*>(d_out->data<T>()) + top_size * i;
    }
  }
  void Compute(const framework::ExecutionContext& ctx) const override {
    auto* bottom0 = ctx.Input<LoDTensor>("X");
    ...
    if (_trans[1] == CblasTrans) {
      ...
      call_gemm_batched(
          ctx, bot0_trans, CblasNoTrans, static_cast<int>(_dims[1]),
          static_cast<int>(_dims[2]), static_cast<int>(_dims[0]), _scale,
