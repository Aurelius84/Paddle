...
class Matrix {
 public:
  Matrix(const T* data, const int rows, const int cols, const int ldx,
         const bool trans)
 ...
 private:
  T& _get(const int i, const int j, int t1 = 0, int t2 = 0, int t3 = 0,
          int t4 = 0) {
    if (i < rows() && j < cols()) {
      return _data[i * _ldx + j];
    }
...
template <typename T>
void gemm(const bool row_major, const bool trans_a, const bool trans_b,
  ...
  if (row_major) {
    for (int row_c = 0; row_c < M; row_c++) {
      for (int col_c = 0; col_c < ldc; col_c++) {
        float a_mul_b = 0;
        if (ma.cols() != mb.rows()) {
          std::cerr << "ma.cols() != mb.rows(): " << ma.cols()
                    << " != " << mb.rows() << std::endl;
          exit(1);
        }
        for (int pos = 0; pos < ma.cols(); pos++) {
          a_mul_b += ma.get(row_c, pos, row_c, col_c, pos) *
                     mb.get(pos, col_c, row_c, col_c, pos);
        }
        mc.get(row_c, col_c) =
            alpha * a_mul_b + beta * mc.get(row_c, col_c, row_c, col_c);
      }
    }
...
template <typename T>
void gemm(const bool trans_a, const bool trans_b, int M, int N, int K, T alpha,
