...
class SearchSeqSoftmaxOP : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    PADDLE_ENFORCE(ctx->HasOutput("Out_log"),
                   "Out_log(Output) should not be null.");
 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (_output_log) {
      ...
      const int size = top0->dims()[0] * top0->dims()[1];
      PADDLE_ENFORCE_EQ(size, _prob->dims()[0] * _prob->dims()[1],
                        "top_size should be eq to _prob_size");
      for (int i = 0; i < size; ++i) {
