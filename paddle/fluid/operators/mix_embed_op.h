...
class MixEmbedKernel : public framework::OpKernel<T> {
 public:
  ...
  void Compute(const framework::ExecutionContext &context) const override {
    ...
    auto *_mix_char_offset = context.Output<Tensor>("mix_char_offset");
    auto *_buffer = context.Output<Tensor>("buffer");
    ...
    output_t->Resize(framework::make_ddim({top_l, _cap_e}));
      ...
      if (w > 0) {
        ...
        for (int j = 0; j < w; ++j) {
          ...
          pooling_ff(_pool_type, max_index, sum_num, sub_data, top_data,
                     (const size_t)top_offset_j, _cap_e);
...
class MixEmbedGradKernel : public framework::OpKernel<T> {
 public:
  ...
  void Compute(const framework::ExecutionContext &context) const override {
    ...
    auto *d_out = context.Input<LoDTensor>(
        framework::GradVarName("Out"));  // float tensor
    auto *_max_index = context.Input<Tensor>("max_index");
